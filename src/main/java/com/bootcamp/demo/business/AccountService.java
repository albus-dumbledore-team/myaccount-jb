...
public class AccountService implements Service<Account> {
    ...
    public enum ErrorCode{
        SERVER(1),
        REPOSITORY(2), // for example in the case one is trying to add an account with a username that already exists
        VALIDATION(3),
    ...
    public String add(final Account account) throws ServiceException {
        try {
            ...
            throw new ServiceException(ErrorCode.VALIDATION,"validation bad");
        } catch (ExecutionException | InterruptedException exception) {
            throw new ServiceException(ErrorCode.SERVER, exception.getMessage());
        }
        catch(Exception e){
            throw new ServiceException(ErrorCode.REPOSITORY, e.getMessage());
        }
    }
    ...
    public String updatePassword(String username, String oldPassword, String newPassword, String confirmNewPassword) throws ServiceException {
        ...
        if(!newPassword.equals(confirmNewPassword)){
            throw new ServiceException(ErrorCode.PASSWORD_VALIDATION, "New password and Confirm new password field do not match");
        }
    ...
    public List<Account> getAll() throws ServiceException {
        try {
            ...
            throw new ServiceException(ErrorCode.SERVER,e.getMessage());
        }
    }
}